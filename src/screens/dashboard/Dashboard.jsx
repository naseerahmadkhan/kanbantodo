...
export default function Dashboard({route,navigation}) {
  ...
  const getDataFromDB = async()=>{
    ...
    if (docSnap.exists()) {
      ...
      jsonData.boardList && jsonData.boardList.map((item,index)=>{
...
await updateDoc(todoRef, {
  ...
  const deleteBoard = async(board) =>{
   ...
   let  updatedBoardList = boardListBeforeDel.filter( function(item) { //callback function
      if(item.boardName != board.boardName) { //filtering criteria
        return item;
      }
    })
      ...
      <ScrollView style={{ flex: 0.9 }}>
        ...
        {boardList && boardList.map((item, index) => {
          return (
            <TouchableRipple
              key={index}
              onPress={()=>navigation.navigate('TaskList',{data:{email:route.params.email,boardName:item.boardName,id:index}})}
  ...
  const createBoardDialog = (
    ...
    />
        ...
        <Dialog.Actions>
          ...
          <Button onPress={() => setShowCreateBoardDialog(false)}>Cancel</Button>
        </Dialog.Actions>
      </Dialog>
    </Portal>
  );
